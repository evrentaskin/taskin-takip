...
const planDateLabel = day => {
  if (!day?.date) return `${day?.day || ''}. Gün`
  const date = new Date(`${day.date}T00:00:00`)
  if (Number.isNaN(date.getTime())) return `${day?.day || ''}. Gün`
...
const normalizePlanDates = (plan, startDate) => {
  if (!Array.isArray(plan) || !plan.length || !startDate) return Array.isArray(plan) ? plan : []
  const start = new Date(`${startDate}T00:00:00`)
  if (Number.isNaN(start.getTime())) return plan
  return plan.map((day,index)=>{
    if (day?.date) return day
    const date=new Date(start); date.setDate(date.getDate()+index)
    return {...day,date:dateKeyLocal(date)}
  })
}
...
export default function LgsPage() {
  ...
  const [smartProgramEnabled, setSmartProgramEnabled] = useState(false)
  const [smartProgramStartDate, setSmartProgramStartDate] = useState('')
  const [smartProgramDuration, setSmartProgramDuration] = useState(14)
  const [smartProgramDailyQuestions, setSmartProgramDailyQuestions] = useState(70)
  const [smartProgramMode, setSmartProgramMode] = useState('teacher')
  const [smartProgramSaving, setSmartProgramSaving] = useState(false)
  ...
  async function loadDashboard() {
    ...
    try {
      ...
      const smartProgram = globalSettings?.payload?.smartProgram || {}
      setSmartProgramEnabled(Boolean(smartProgram.enabled))
      setSmartProgramStartDate(smartProgram.startDate || '')
      setSmartProgramDuration(Number(smartProgram.duration) || 14)
      setSmartProgramDailyQuestions(Number(smartProgram.dailyQuestions) || 70)
      setSmartProgramMode(smartProgram.mode === 'auto' ? 'auto' : 'teacher')
      ...
      if (lgsClass?.id) {
        ...
        if (portalData?.length) {
          ...
          const dbPlans = Object.fromEntries(portalData.map(row => [row.student_id, normalizePlanDates(row.study_plan || [], smartProgram.startDate)]))
  ...
  async function toggleSmartProgram(nextEnabled) {
    setSmartProgramSaving(true)
    try {
      const duration = Math.max(1, Number(smartProgramDuration) || 14)
      const dailyQuestions = Math.max(40, Math.min(120, Number(smartProgramDailyQuestions) || 70))
      const startDate = smartProgramStartDate || dateKeyLocal(new Date())
      ...
      const smartProgram = {
        ...
        duration,
        dailyQuestions,
        mode: smartProgramMode === 'auto' ? 'auto' : 'teacher',
        updatedAt: now
      }

      // Ayar değiştirip yeniden başlatıldığında eski programı kullanma:
      // tüm öğrenciler için yeni dönem programını baştan üret.
      if (nextEnabled && lgsStudents.length) {
        const regeneratedPlans = {}
        const regeneratedDates = {}
        lgsStudents.forEach(student => {
          const results = resultsByStudent.get(student.id) || []
          const plan = buildStudyPlan({ ...student, results })
          regeneratedPlans[student.id] = plan
          regeneratedDates[student.id] = now
        })

        setStudyPlans(regeneratedPlans)
        setStudyPlanDates(regeneratedDates)
        localStorage.setItem('lgsStudyPlans', JSON.stringify(regeneratedPlans))
        localStorage.setItem('lgsStudyPlanDates', JSON.stringify(regeneratedDates))
        window.dispatchEvent(new Event('taskin-lgs-study-program-updated'))
        ...
        const rowsToPersist = lgsStudents.map(student => ({
          ...
          study_plan: regeneratedPlans[student.id] || [],
          study_plan_generated_at: regeneratedDates[student.id],
          updated_at: now
        }))

        const { error: planError } = await supabase
          .from('lgs_student_portal_settings')
          .upsert(rowsToPersist, { onConflict: 'student_id' })

        if (planError) throw planError
      }

      const current = await readSharedState('lgs-global-settings-v1', {})
      const payload = { ...(current.payload || {}), lgsDate, smartProgram }
      await writeSharedState('lgs-global-settings-v1', payload)

      setSmartProgramEnabled(Boolean(nextEnabled))
      setSmartProgramStartDate(startDate)
      localStorage.setItem('lgsSmartProgram', JSON.stringify(smartProgram))

      setMessage(
        nextEnabled
          ? `${duration} günlük program başlatıldı. ${lgsStudents.length} öğrencinin programı yeniden oluşturuldu.`
          : 'Akıllı çalışma programı durduruldu.'
      )
    } catch (saveError) {
      setError(`Akıllı çalışma programı kaydedilemedi: ${saveError?.message || saveError}`)
    } finally {
      setSmartProgramSaving(false)
    }
  }
  ...
  async function saveSettings() {
    ...
    try {
      const smartProgram = {
        enabled: Boolean(smartProgramEnabled),
        startDate: smartProgramStartDate || '',
        duration: Number(smartProgramDuration) || 14,
        dailyQuestions: Math.max(40, Math.min(120, Number(smartProgramDailyQuestions) || 70)),
        mode: smartProgramMode === 'auto' ? 'auto' : 'teacher',
        updatedAt: new Date().toISOString()
      }
      await writeSharedState('lgs-global-settings-v1', { lgsDate, smartProgram })
      localStorage.setItem('lgsSmartProgram', JSON.stringify(smartProgram))
    } catch (cloudError) {
  ...
  function getSmartProgramAnalysisResults(student) {
    const all = [...(student.results || [])]
      .filter(row => row && row._examDate)
      .sort((a,b) => String(a._examDate).localeCompare(String(b._examDate)))

    if (!all.length) return []

    const duration = Math.max(1, Number(smartProgramDuration) || 14)
    const start = smartProgramStartDate
      ? new Date(`${smartProgramStartDate}T00:00:00`)
      : new Date()

    if (Number.isNaN(start.getTime())) return all.slice(-3)

    // Yeni programın analiz dönemi, yeni program başlamadan hemen önceki
    // aynı uzunluktaki dönemdir. Örn. 15-28 Ağustos programı için 1-14 Ağustos.
    const previousStart = new Date(start)
    previousStart.setDate(previousStart.getDate() - duration)

    const inPreviousPeriod = all.filter(row => {
      const d = new Date(`${String(row._examDate).slice(0,10)}T00:00:00`)
      return d >= previousStart && d < start
    })

    // Önceki dönemde hiç deneme yoksa başlangıç programı kullanılacak.
    // Geçmişte deneme var ama tarih aralığı dışında ise bunları yeni programa
    // yanlışlıkla taşımıyoruz.
    return inPreviousPeriod
  }

  function buildStudyPlan(student) {
    const results = getSmartProgramAnalysisResults(student)
    const duration = Math.max(1, Number(smartProgramDuration) || 14)
    const dailyTarget = Math.max(40, Math.min(120, Number(smartProgramDailyQuestions) || 70))

    // Okul + online denemelerin tamamı aynı analiz havuzunda değerlendirilir.
    // Her deneme bir ölçüm kabul edilir; ders başarısı LGS soru sayısına göre normalize edilir.
    const performance = lessonDefs.map(lesson => {
      const values = results
        .map(row => Number(row[`${lesson.key}_net`]))
        .filter(Number.isFinite)
      const ratio = values.length
        ? Math.max(0, Math.min(1, average(values) / lesson.count))
        : null
      return {
        ...lesson,
        ratio,
        attempts: values.length,
        averageNet: values.length ? average(values) : null
      }
    })

    const weakness = new Map(performance.map(item => [
      item.key,
      item.ratio == null ? 0.45 : Math.max(0.05, 1 - item.ratio)
    ]))

    const main = ['math','science','turkish']
    const support = ['history','english','religion']

    // Deneme sonucu olmayan derse başlangıç ağırlığı verilir.
    // Sonuç olan derslerde zayıflık arttıkça ağırlık artar.
    const scoreFor = key => {
      const weak = weakness.get(key) || 0
      const mainBonus = main.includes(key) ? 0.55 : 0
      return weak + mainBonus
    }

    const rankedMain = [...main].sort((a,b) => scoreFor(b) - scoreFor(a))
    const rankedSupport = [...support].sort((a,b) => scoreFor(b) - scoreFor(a))

    const patterns = []
    for (let day = 0; day < duration; day += 1) {
      // Ana derslerden biri mutlaka her gün gelir; ikinci ana ders
      // zayıflık ve gün rotasyonuna göre değişir.
      const firstMain = rankedMain[day % rankedMain.length]
      const secondCandidates = rankedMain.filter(key => key !== firstMain)
      const secondMain = secondCandidates[(Math.floor(day / rankedMain.length) + day) % secondCandidates.length]

      // Destek dersleri her gün değil, dönüşümlü gelir.
      // Daha zayıf destek dersleri daha erken/sık seçilir.
      const supportKey = day % 2 === 0
        ? rankedSupport[(Math.floor(day / 2)) % rankedSupport.length]
        : rankedSupport[(Math.floor(day / 2) + 1) % rankedSupport.length]

      const pattern = [firstMain, secondMain]
      if (day % 3 !== 1 || results.length === 0) pattern.push(supportKey)

      // Aynı kombinasyonun arka arkaya gelmesini önle.
      if (patterns.length && pattern.join('|') === patterns.at(-1).join('|')) {
        pattern.reverse()
      }
      patterns.push([...new Set(pattern)].slice(0, 3))
    }

    const allocate = (keys, total) => {
      const weights = keys.map(key => {
        const weaknessWeight = weakness.get(key) || 0.45
        const mainBase = main.includes(key) ? 1.55 : 0.8
        return mainBase + weaknessWeight * 1.8
      })
      const sum = weights.reduce((a,b)=>a+b,0) || 1
      const values = keys.map((key,index) => ({
        key,
        value: Math.max(5, Math.floor(total * weights[index] / sum / 5) * 5)
      }))

      let used = values.reduce((s,x)=>s+x.value,0)
      while (used < total) {
        const target = values
          .slice()
          .sort((a,b)=>(weakness.get(b.key)||0)-(weakness.get(a.key)||0))[0]
        target.value += 5
        used += 5
      }
      while (used > total) {
    ...
    const studentSeed = String(student.id || student.student_number || '')
      .split('')
      .reduce((sum,ch)=>sum + ch.charCodeAt(0), 0)

    const rotation = duration > 0 ? studentSeed % Math.max(1, patterns.length) : 0
    const rotatedPatterns = patterns.map((_, index) => patterns[(index + rotation) % patterns.length])

    return Array.from({ length: duration }, (_, index) => {
      let pattern = [...(rotatedPatterns[index] || patterns[index % patterns.length])]

      // Çok zayıf bir ders varsa, üçlü rotasyonda değilse uygun günlerde eklenir.
      if (results.length && index % 3 === 0) {
        const weakest = [...weakness.entries()]
          .sort((a,b)=>b[1]-a[1])
          .map(([key])=>key)
          .find(key => !pattern.includes(key))
        if (weakest) {
          pattern = [...pattern.slice(0, 2), weakest]
        }
      }

      pattern = [...new Set(pattern)].slice(0, 3)
      const allocations = allocate(pattern, dailyTarget)

      const dayItems = allocations.map(({key,value}) => {
        const lesson = lessonDefs.find(item=>item.key===key)
        return {
          id:`${student.id}-${stamp}-${index}-${key}`,
          lesson:lesson?.name || key,
          task:`${value} soru`,
          questions:value,
          done:false
        }
      ...
      // Her 3 günde bir konu adı olmadan tekrar.
      if (index % 3 === 2) {
        const reviewKey = pattern[index % pattern.length]
        const lesson = lessonDefs.find(item=>item.key===reviewKey)
        dayItems.push({
          id:`${student.id}-${stamp}-${index}-review-${reviewKey}`,
          lesson:lesson?.name || reviewKey,
          task:'Tekrar',
          questions:0,
          done:false,
          isReview:true
      ...
      const planStart = smartProgramStartDate || dateKeyLocal(new Date())
      const planDate = new Date(`${planStart}T00:00:00`)
      if (!Number.isNaN(planDate.getTime())) planDate.setDate(planDate.getDate() + index)
      return {
        day:index + 1,
        date:dateKeyLocal(planDate),
        items:dayItems,
        analysisExamCount:results.length,
        analysisSources:{
          total:results.length,
          school:results.filter(row=>row._kind==='normal').length,
          online:results.filter(row=>row._kind==='online').length
        }
      }
    })
  }
  ...
  function generateStudyPlan(student, notify=true) {
    ...
    if (notify) setMessage(`${plan.length} günlük çalışma programı otomatik olarak hazırlandı.`)
  }

  useEffect(() => {
    if (loading || !lgsStudents.length) return
    const programDurationMs = Math.max(1, Number(smartProgramDuration) || 14) * 24 * 60 * 60 * 1000
    const now = Date.now()
    let changed = false
    ...
    lgsStudents.forEach(student => {
      ...
      if (!existing.length || !generatedAt || now - generatedAt >= programDurationMs) {
  ...
  }, [loading, lgsStudents, resultsByStudent, smartProgramDuration, smartProgramDailyQuestions])
  ...
  async function downloadStudyPlanPdf(student, planOverride=null) {
    const plan = planOverride || studyPlans[student.id] || []
    if (!plan.length) { setError('PDF için çalışma programı bulunamadı.'); return }
    const fullName=`${student.first_name||''} ${student.last_name||''}`.trim()
    const daysHtml=plan.map(day=>`<div class="plan-day"><div class="day-title">${escapeHtml(planDateLabel(day))}</div><div class="day-total">${(day.items||[]).reduce((sum,x)=>sum+Number(x.questions||0),0)} soru</div><div class="items">${(day.items||[]).map(item=>`<div class="item"><b>${escapeHtml(item.lesson)}</b><span>${Number(item.questions||0)>0?`${Number(item.questions||0)} soru`:'Tekrar'}</span></div>`).join('')}</div></div>`).join('')
    const html=`<style>*{box-sizing:border-box}body{margin:0;font-family:Arial;color:#172b3a}.report-page{width:1120px;height:790px;padding:22px;background:#fff;overflow:hidden}.head{display:flex;justify-content:space-between;align-items:center;border-bottom:2px solid #174c67;padding-bottom:8px;margin-bottom:10px}.head h1{margin:0;font-size:22px;color:#174c67}.meta{font-size:11px;text-align:right}.plan-grid{display:grid;grid-template-columns:repeat(4,1fr);grid-auto-rows:1fr;gap:6px;height:710px}.plan-day{border:1px solid #cbd8df;border-radius:7px;padding:6px;overflow:hidden;background:#fff}.day-title{font-size:11px;font-weight:800;color:#174c67;line-height:1.15}.day-total{font-size:8px;color:#5b6c75;margin:2px 0 4px}.item{display:flex;justify-content:space-between;gap:4px;font-size:8.5px;line-height:1.2;padding:2px 0;border-top:1px solid #edf1f3}.item b{font-size:8.5px}.item span{font-weight:700;white-space:nowrap}.foot{font-size:8px;color:#667;margin-top:4px}</style><section class="report-page"><div class="head"><div><h1>Çalışma Programı</h1><div class="meta">${escapeHtml(fullName)} • LGS Grubu</div></div><div class="meta">${plan.length} günlük program</div></div><div class="plan-grid">${daysHtml}</div><div class="foot">Ders • soru sayısı • tekrar</div></section>`
    await runPdfDownload(html,`${fileSafe(fullName)}_calisma_programi.pdf`,'landscape')
    setMessage('Çalışma programı PDF olarak indirildi.')
  }
  ...
  async function downloadClassReportPdf() {
    ...
    const html=`${pdfStyles}<section class="report-page"><div class="report-head"><div class="brand-lock"><img class="brand-pdf-logo" src="/taskin-logo-horizontal.png" alt="TAŞKIN"><div><div class="brand-sub">LGS Sınıf Genel Başarı Raporu</div></div></div><div class="report-center"><h1>Ders Bazlı Öğrenci Ortalamaları</h1><b>LGS Grubu</b></div><div class="report-meta"><div><b>Öğrenci:</b> ${classReportRows.length}</div><div><b>Sıralama:</b> Puan ortalaması</div><div><b>Rapor Tarihi:</b> ${new Date().toLocaleDateString('tr-TR')}</div></div></div><table class="result-table"><thead><tr><th rowspan="2">Sıra</th><th rowspan="2">No</th><th rowspan="2">Adı Soyadı</th><th rowspan="2">Deneme</th>${headers}<th rowspan="2">Toplam Net Ort.</th><th rowspan="2">Puan Ort.</th></tr><tr class="sub">${sub}</tr></thead><tbody>${rows}${avgRow}</tbody></table></section>`
  ...
  return <Box className="lgs-dashboard-page">
    ...
    {loading ? <Box className="loader compact"><CircularProgress/></Box> : studentSummaries.length ? (
      <Box className="lgs-student-card-grid">
        {studentSummaries.map(student => <button className="glass lgs-student-card" key={student.id} onClick={()=>openStudent(student)}>
          ...
          <div className="lgs-progress-line"><span>Çalışma programı</span><b>%{student.progress}</b></div>
    ...
    <Dialog open={settingsOpen} onClose={()=>setSettingsOpen(false)} fullWidth maxWidth="md">
      ...
      <DialogContent>
        ...
        <Paper variant="outlined" sx={{ p:2, mb:2, borderRadius:3, bgcolor:'#fafcff' }}>
          <Stack direction={{ xs:'column', sm:'row' }} justifyContent="space-between" alignItems={{ xs:'stretch', sm:'center' }} spacing={1} sx={{ mb:2 }}>
            <Box>
              <Stack direction="row" spacing={1} alignItems="center">
                ...
                <Typography variant="h6" fontWeight={900}>Akıllı Çalışma Programı</Typography>
              </Stack>
              <Typography variant="body2" color="text.secondary">Deneme sonuçlarına göre dönemsel öğrenci çalışma programı.</Typography>
            </Box>
            <Chip color={smartProgramEnabled ? 'success' : 'default'} label={smartProgramEnabled ? 'Aktif' : 'Durduruldu'} />
          </Stack>

          <Stack spacing={2}>
            <TextField fullWidth label="Program başlangıç tarihi" type="date" value={smartProgramStartDate} onChange={e=>setSmartProgramStartDate(e.target.value)} InputLabelProps={{ shrink:true }} disabled={smartProgramEnabled} />
            <TextField fullWidth select SelectProps={{ native:true }} label="Program süresi" value={smartProgramDuration} onChange={e=>setSmartProgramDuration(Number(e.target.value))} disabled={smartProgramEnabled}>
              <option value={7}>7 Gün</option>
              <option value={14}>14 Gün</option>
              <option value={21}>21 Gün</option>
              <option value={28}>28 Gün</option>
            </TextField>
            <TextField fullWidth select SelectProps={{ native:true }} label="Günlük hedef soru sayısı" value={smartProgramDailyQuestions} onChange={e=>setSmartProgramDailyQuestions(Number(e.target.value))} disabled={smartProgramEnabled}>
              <option value={50}>50 Soru</option>
              <option value={60}>60 Soru</option>
              <option value={70}>70 Soru</option>
              <option value={80}>80 Soru</option>
              <option value={90}>90 Soru</option>
              <option value={100}>100 Soru</option>
              <option value={120}>120 Soru</option>
            </TextField>
            <TextField fullWidth select SelectProps={{ native:true }} label="Program oluşturma" value={smartProgramMode} onChange={e=>setSmartProgramMode(e.target.value)} disabled={smartProgramEnabled}>
              <option value="teacher">Öğretmen Onaylı</option>
              <option value="auto">Otomatik</option>
            </TextField>
            <Stack direction={{ xs:'column', sm:'row' }} spacing={1}>
              <Button fullWidth color="success" variant="contained" startIcon={<PlayCircle/>} disabled={smartProgramEnabled || smartProgramSaving} onClick={()=>toggleSmartProgram(true)}>
                {smartProgramSaving ? 'Kaydediliyor...' : 'Programı Başlat'}
              </Button>
              <Button fullWidth color="error" variant="outlined" startIcon={<RestartAlt/>} disabled={!smartProgramEnabled || smartProgramSaving} onClick={()=>toggleSmartProgram(false)}>
                Programı Durdur
              </Button>
            </Stack>
          </Stack>
        ...
        {selectedSummary && <Stack spacing={2}>
          ...
          <Box className="glass lgs-study-section">
            <Box className="section-heading">
              <Box><Stack direction="row" spacing={1} alignItems="center"><Psychology color="primary"/><Typography variant="h6" fontWeight={950}>Yapay Zekâ Destekli Çalışma Programı</Typography></Stack><Typography color="text.secondary">Program seçilen süre ve günlük soru hedefine göre otomatik hazırlanır; dersler dönüşümlü gelir ve belirli günlerde tekrar eklenir.</Typography></Box>
              <Stack direction={{xs:'column',sm:'row'}} spacing={1} alignItems={{sm:'center'}}><Chip color="success" icon={<AutoAwesome/>} label="Otomatik program" />{selectedPlan.length>0&&<Button size="small" variant="outlined" startIcon={<PictureAsPdf/>} onClick={()=>downloadStudyPlanPdf(selectedSummary)}>PDF İndir</Button>}</Stack>
            </Box>
            {selectedPlan.length ? <>
              ...
              <Box className="lgs-plan-days">{selectedPlan.map(day => <Box className="lgs-plan-day" key={day.day}><Typography fontWeight={950}>{planDateLabel(day)} <small>• Toplam {(day.items||[]).reduce((sum,x)=>sum+Number(x.questions||0),0)} soru</small></Typography>{(day.items||[]).map(item=><label className={`lgs-plan-item ${item.done?'done':''}`} key={item.id}><Checkbox checked={item.done} onChange={()=>togglePlanItem(selectedSummary.id,item.id)}/><div><b>{item.lesson}</b><span>{item.task}</span><small>{item.questions?`${item.questions} soru`:''}</small></div>{item.done&&<CheckCircle color="success"/>}</label>)}</Box>)}</Box>
            </> : <Box className="empty-chart"><AutoAwesome/><Typography fontWeight={900}>Çalışma programı hazırlanıyor</Typography><Typography color="text.secondary">Öğrenci verileri yüklendiğinde Seçilen süreye göre plan otomatik oluşturulur.</Typography></Box>}
          </Box>
        </Stack>}
      </DialogContent>
    </Dialog>
