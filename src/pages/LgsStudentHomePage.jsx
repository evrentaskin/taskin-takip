...
import {
  ...
  Logout, Settings, TrendingDown, TrendingUp, OnlinePrediction, PlayArrow, Save, PictureAsPdf
...
export default function LgsStudentHomePage({ session, student, classInfo }) {
  ...
  useEffect(()=>{
    const refreshPlan = async () => {
      try {
        const { data, error } = await supabase.rpc('get_my_lgs_portal_settings')
        let row = Array.isArray(data) ? data[0] : data
        const rpcPlan = Array.isArray(row?.study_plan) ? row.study_plan : []
        if (error || !row || !rpcPlan.length) {
          const fallback = await supabase
            ...
            .maybeSingle()
          if (!fallback.error && fallback.data) row = fallback.data
        }
        if (row) {
          const global = await readSharedState('lgs-global-settings-v1', {})
          const startDate = global?.payload?.smartProgram?.startDate || null
          setPortal(prev => ({
            ...prev,
            target_score: row.target_score ?? prev.target_score ?? null,
            target_history: row.target_history || prev.target_history || [],
            study_plan: normalizePlanDates(Array.isArray(row.study_plan) ? row.study_plan : [], startDate),
            study_plan_generated_at: row.study_plan_generated_at || null
          }))
        }
      } catch {}
    }
    const timer = window.setInterval(refreshPlan, 15000)
  ...
  async function load(){
    ...
    const rpcPlan = Array.isArray(portalData?.study_plan) ? portalData.study_plan : []
    if (portalRpcRes.error || !portalData || !rpcPlan.length) {
      const fallback = await supabase
        .from('lgs_student_portal_settings')
        .select('student_id,target_score,target_history,study_plan,study_plan_generated_at')
        .eq('student_id',student.id)
        .maybeSingle()
      if (!fallback.error && fallback.data) portalData = fallback.data
    }
    if(portalData){
      const global = await readSharedState('lgs-global-settings-v1', {})
      const startDate = global?.payload?.smartProgram?.startDate || null
      setPortal({
        target_score:portalData.target_score??null,
        target_history:portalData.target_history||[],
        study_plan:normalizePlanDates(Array.isArray(portalData.study_plan)?portalData.study_plan:[], startDate),
        study_plan_generated_at:portalData.study_plan_generated_at||null
      })
    }
  ...
  async function downloadStudyPlanPdf(){
    ...
    const daysHtml=plan.map(day=>`<div class="plan-day"><div class="day-title">${esc(planDateLabel(day))}</div><div class="day-total">${(day.items||[]).reduce((sum,x)=>sum+Number(x.questions||0),0)} soru</div><div class="items">${(day.items||[]).map(item=>`<div class="item"><b>${esc(item.lesson)}</b><span>${Number(item.questions||0)>0?`${Number(item.questions||0)} soru`:'Tekrar'}</span></div>`).join('')}</div></div>`).join('')
    const html=`<style>*{box-sizing:border-box}body{margin:0;font-family:Arial;color:#172b3a}.report-page{width:1120px;height:790px;padding:22px;background:#fff;overflow:hidden}.head{display:flex;justify-content:space-between;align-items:center;border-bottom:2px solid #174c67;padding-bottom:8px;margin-bottom:10px}.head h1{margin:0;font-size:22px;color:#174c67}.meta{font-size:11px;text-align:right}.plan-grid{display:grid;grid-template-columns:repeat(4,1fr);grid-auto-rows:1fr;gap:6px;height:710px}.plan-day{border:1px solid #cbd8df;border-radius:7px;padding:6px;overflow:hidden}.day-title{font-size:11px;font-weight:800;color:#174c67;line-height:1.15}.day-total{font-size:8px;color:#5b6c75;margin:2px 0 4px}.item{display:flex;justify-content:space-between;gap:4px;font-size:8.5px;line-height:1.2;padding:2px 0;border-top:1px solid #edf1f3}.item b{font-size:8.5px}.item span{font-weight:700;white-space:nowrap}.foot{font-size:8px;color:#667;margin-top:4px}</style><section class="report-page"><div class="head"><div><h1>Çalışma Programım</h1><div class="meta">${esc(fullName)} • LGS Grubu</div></div><div class="meta">${plan.length} günlük program</div></div><div class="plan-grid">${daysHtml}</div><div class="foot">Ders • soru sayısı • tekrar</div></section>`
    const host=document.createElement('div');host.innerHTML=html;Object.assign(host.style,{position:'fixed',left:'0',top:'0',width:'1120px',background:'#fff',zIndex:'2147483646'});document.body.appendChild(host)
    try{await html2pdf().set({margin:0,filename:`${fullName.replace(/[^a-z0-9ğüşöçıİĞÜŞÖÇ _-]/gi,'_')}_calisma_programi.pdf`,image:{type:'jpeg',quality:.98},html2canvas:{scale:2,useCORS:true,backgroundColor:'#fff',width:1120,height:790,windowWidth:1120,windowHeight:790},jsPDF:{unit:'mm',format:'a4',orientation:'landscape'},pagebreak:{mode:['css']}}).from(host).save()}finally{host.remove()}
  ...
  return <Box className="lgs-student-shell">
    <Box className="lgs-student-sidebar">
      <Box className="brand"><img className="brand-logo-image" src="/taskin-logo-symbol.png" alt="TAŞKIN"/><Box><Typography fontWeight={950}>TAŞKIN</Typography><Typography variant="caption">LGS Öğrenci Paneli</Typography></Box></Box>
    ...
    <Box className="lgs-student-main">
      ...
      {page==='Çalışma Programım'&&<>
        <Stack direction={{xs:'column',sm:'row'}} spacing={1} alignItems={{sm:'center'}} justifyContent="space-between" sx={{mb:2}}><Stack direction="row" spacing={1} alignItems="center"><CalendarMonth/><Typography variant="h4" fontWeight={950}>{plan.length ? `${plan.length} Günlük Çalışma Programım` : 'Çalışma Programım'}</Typography></Stack>{plan.length>0&&<Button variant="outlined" startIcon={<PictureAsPdf/>} onClick={downloadStudyPlanPdf}>PDF İndir</Button>}</Stack>
        {plan.length?<Box className="lgs-student-plan-grid">{plan.map(day=><Paper className="lgs-student-plan-day" key={day.day} elevation={0}><Typography variant="h6" fontWeight={950}>{planDateLabel(day)}</Typography>{(day.items||[]).map(item=><label className={`lgs-student-plan-item ${item.done?'done':''}`} key={item.id}><Checkbox checked={Boolean(item.done)} onChange={()=>togglePlan(item.id)}/><Box><b>{item.lesson}</b><span>{item.task}</span>{Number(item.questions||0)>0&&<small>{item.questions} soru</small>}</Box>{item.done&&<CheckCircle color="success"/>}</label>)}</Paper>)}</Box>:<Paper className="lgs-panel" elevation={0}><Typography color="text.secondary">Öğretmenin henüz çalışma programı oluşturmadı.</Typography></Paper>}
      </>}
    </Box>
