...
export default function PrivateLessonsPage(){
  ...
  const [schoolForm,setSchoolForm]=useState({name:'',date:'',correct:'',wrong:'',net:'',schoolRank:''})
  ...
  function openSchool(item=null){setSchoolForm(item?{...item}:{name:'',date:'',correct:'',wrong:'',net:'',schoolRank:''});setSchoolOpen(true)}
  ...
  return <Box sx={{p:{xs:1.25,md:2}}}>
    <Stack spacing={2}>
      <Paper className="glass" sx={{p:2,borderRadius:3}}>
        <Stack direction={{xs:'column',sm:'row'}} spacing={1} alignItems={{sm:'center'}} justifyContent="space-between"><Box><Typography variant="h5" fontWeight={950}>Özel Ders Öğrencileri</Typography><Typography variant="body2" color="text.secondary">Öğrenciler yukarıda; seçilen öğrencinin tüm bilgileri sayfanın tamamında görünür.</Typography></Box><Button variant="contained" startIcon={<PersonAdd/>} onClick={openNew}>Öğrenci Ekle</Button></Stack>
      ...
      {!selected?<Alert severity="info">Detayları görmek için bir öğrenci seç.</Alert>:<Paper className="glass" variant="outlined" sx={{p:0,borderRadius:3,overflow:'hidden',borderWidth:2,borderColor:'#f59e0b'}}>
        ...
        <Divider/>
        <Tabs value={tab} onChange={(_,v)=>setTab(v)} variant="scrollable" scrollButtons="auto"><Tab label="Takvim ve Ücret"/><Tab label="Online Denemeler"/><Tab label="Okul Denemeleri"/><Tab label="Analiz"/></Tabs>
        <Box sx={{p:2}}>
