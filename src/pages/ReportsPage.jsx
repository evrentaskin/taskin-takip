...
export default function ReportsPage() {
  ...
  const [exams] = useSharedCloudState({ stateKey:'exams-v1', localKey:EXAMS_KEY, fallback:safeLoad(EXAMS_KEY), readOnly:true, onError:e=>setError(e?.message || 'Denemeler buluttan yüklenemedi.') })
  const [homeworks] = useSharedCloudState({ stateKey:'homeworks-v1', localKey:HOMEWORK_KEY, fallback:safeLoad(HOMEWORK_KEY), readOnly:true, onError:e=>setError(e?.message || 'Ödevler buluttan yüklenemedi.') })
  ...
  function questionAnalysis() {
    ...
    return Array.from({length:Number(selectedExam?.questionCount || selectedExam?.question_count || 20)},(_,i)=>{const q=i+1; let correct=0,wrong=0,blank=0; const key=selectedExam.answers?.[q] ?? selectedExam.answers?.[String(q)]; rows.forEach(r=>{const ans=r.answers?.[q] ?? r.answers?.[String(q)]; if(!ans) blank++; else if(ans===key) correct++; else wrong++}); const pct=rows.length?Math.round(correct*100/rows.length):0; return {q,correct,wrong,blank,pct} })
  }
  ...
  function exportPdf() {
    ...
    html2pdf().set({
      ...
      pagebreak:{mode:['css','legacy'], before:['.pdf-page-start'], avoid:['.report-letterhead','.report-table tr','.question-box','.mini-analysis']},
      jsPDF:{unit:'mm',format:'a4',orientation:isLandscapeReport()?'landscape':'portrait'}
  ...
  return <Box className="reports-page">
    ...
    <Box ref={reportRef} className={`print-report unified-pdf-report ${isLandscapeReport()?'pdf-landscape-report':'pdf-portrait-report'}`}>
      {renderReportPages()}
    </Box>
  </Box>


  function ReportHeader() { return <Box className="report-letterhead unified-report-letterhead"><Box className="report-logo report-logo-wide"><img src="/taskin-logo-horizontal.png" alt="TAŞKIN logosu"/></Box><Typography className="report-head-title" fontWeight={900}>{reportTitle}</Typography><Box className="report-meta"><b>{classInfo?.name}</b>{mode==='single' && ['fen','general','online'].includes(section) && <span>{selectedExam?.name || ''}</span>}<span>{new Date().toLocaleDateString('tr-TR')}</span></Box></Box> }
  function ReportPage({page, children}) { return <Box className={`pdf-report-page ${page > 1 ? 'pdf-page-start' : ''}`}><ReportHeader/><Box className="pdf-page-content">{children}</Box><Box className="report-footer">TAŞKIN • Oluşturulma: {new Date().toLocaleString('tr-TR')} • Sayfa {page}</Box></Box> }
  function isLandscapeReport() {
    if (section === 'homework') return true
    if (mode === 'all' && ['fen','general','online'].includes(section)) return true
    return false
  }
  ...
  function renderReportPages() {
    if (section === 'online') return renderOnlinePages()

    if (section === 'monthly' || section === 'term') {
      const rows = scoreRows(section)
      const pages = chunkRows(rows, 45)
      return <>{pages.map((pageRows, pageIndex)=><ReportPage key={`${section}-${pageIndex}`} page={pageIndex+1}>
        <ReportTable headers={['Sıra','No','Öğrenci','Puan']} rows={pageRows.map((r,i)=>[pageIndex*45+i+1,r.number,r.name,r.score])}/>
      </ReportPage>)}</>
    }

    if (section === 'homework') {
      const allHomework = classHomeworks()
      const homeworkGroups = allHomework.length ? chunkRows(allHomework, 6) : [[]]
      const rows = homeworkRows()
      const studentPages = chunkRows(rows, 45)
      let pageNo = 0
      return <>{homeworkGroups.flatMap((group, groupIndex)=>studentPages.map((pageRows, studentPageIndex)=>{
        pageNo += 1
        const indexes = group.map(h=>allHomework.findIndex(x=>x.id===h.id))
        return <ReportPage key={`hw-${groupIndex}-${studentPageIndex}`} page={pageNo}>
          <ReportTable statusColors headers={['Sıra','No','Öğrenci',...group.map(h=>h.title),'Yaptı','Yapmadı','Gelmedi']} rows={pageRows.map((r,i)=>[studentPageIndex*45+i+1,r.number,r.name,...indexes.map(idx=>r.items[idx]),r.done,r.missing,r.absent])}/>
        </ReportPage>
      }))}</>
    }
    ...
    if (mode === 'all') {
      ...
      const analysisPageNo = pages.length + 1
      return <>
        {pages.map((pageRows,pageIndex)=><ReportPage key={`all-${pageIndex}`} page={pageIndex+1}>
          <ReportTable headers={['Sıra','No','Öğrenci',...relevantExams.map(e=>e.name),'Girilen','Toplam','Ortalama']} rows={pageRows.map((r,i)=>[pageIndex*45+i+1,r.number,r.name,...r.nets.map(v=>v==null?'Girmedi':fmt(v)),`${r.enteredCount}/${relevantExams.length}`,fmt(r.total),fmt(r.avg)])}/>
        </ReportPage>)}
        <ReportPage page={analysisPageNo}>
          <Typography variant="h6" fontWeight={950} sx={{mb:1}}>Deneme Özeti</Typography>
          <Box className="pdf-final-stats"><Box className="report-summary report-stats-summary collective-summary"><Chip icon={<Groups/>} label={`${rows.length} öğrenci`}/><Chip label={`${relevantExams.length} deneme`}/><Chip label={`Genel Ort. Net: ${fmt(overallAvg)}`}/></Box></Box>
        </ReportPage>
      </>
    }
    ...
    const analysisPageNo = pages.length + 1
    return <>
      {pages.map((pageRows,pageIndex)=><ReportPage key={`normal-${pageIndex}`} page={pageIndex+1}>
        <ReportTable headers={['Sıra','No','Öğrenci','Doğru','Yanlış','Net','Hedef Durumu']} rows={pageRows.map((r,i)=>{ const target=Number(selectedExam?.targets?.[r.studentId]); return [pageIndex*45+i+1,r.number,r.name,r.correct,r.wrong,fmt(r.net),Number.isFinite(target)&&target>0?(r.net>=target?'Hedefi Geçti ✓':'Hedefin Altında ✕'):'Hedef Yok'] })}/>
      </ReportPage>)}
      <ReportPage page={analysisPageNo}>
        <Typography variant="h6" fontWeight={950} sx={{mb:1}}>Deneme Analizi</Typography>
  ...
  function renderOnlinePages() {
    if (mode === 'all') {
      ...
      const analysisPageNo = pages.length + 1
      return <>
        {pages.map((pageRows,pageIndex)=><ReportPage key={`online-all-${pageIndex}`} page={pageIndex+1}><ReportTable headers={['Sıra','No','Öğrenci',...chosen.map(e=>e.name),'Girilen','Ortalama']} rows={pageRows.map((r,i)=>[pageIndex*45+i+1,r.number,r.name,...r.nets.map(v=>v==null?'Girmedi':fmt(v)),`${r.enteredCount}/${chosen.length}`,fmt(r.avg)])}/></ReportPage>)}
        <ReportPage page={analysisPageNo}>
          <Typography variant="h6" fontWeight={950} sx={{mb:1}}>Deneme Özeti</Typography>
          <Box className="pdf-final-stats"><Box className="report-summary report-stats-summary collective-summary"><Chip icon={<Groups/>} label={`${rows.length} öğrenci`}/><Chip label={`${chosen.length} deneme`}/><Chip label={`Genel Ort. Net: ${fmt(overallAvg)}`}/></Box></Box>
        </ReportPage>
      </>
    }
    ...
    return <>
      ...
      <ReportPage page={analysisPage}>
        ...
        <Box className="question-grid">{qa.map(x=><Box key={x.q} className={`question-box ${x.pct>=80?'good':x.pct>=50?'mid':'bad'}`}><b>{x.q}</b><span>%{x.pct}</span><small>{x.correct}D / {x.wrong}Y / {x.blank}B</small></Box>)}</Box>
