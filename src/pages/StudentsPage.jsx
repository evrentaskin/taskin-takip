...
async function fetchStudentActivityEvents(userIds, { start = null, end = null } = {}) {
  if (!userIds?.length) return []
  const events = []

  let activityQuery = supabase
    .from('student_activity_events')
    .select('user_id,active_at,activity_type,device_type')
    .in('user_id', userIds)
  if (start) activityQuery = activityQuery.gte('active_at', start.toISOString())
  if (end) activityQuery = activityQuery.lt('active_at', end.toISOString())
  const activityResult = await activityQuery.order('active_at', { ascending: false })
  if (!activityResult.error) {
    events.push(...(activityResult.data || []).map(item => ({
      user_id: item.user_id,
      event_at: item.active_at,
      activity_type: item.activity_type || 'app_active',
      device_type: item.device_type || 'bilgisayar'
    })))
  } else if (!String(activityResult.error.message || '').toLowerCase().includes('does not exist')) {
    console.warn('Aktiflik kayıtları alınamadı', activityResult.error)
  }

  // Eski giriş kayıtlarını da kullan: geçiş gününde / geçmiş raporlarda veri kaybı olmaz.
  let loginQuery = supabase
    .from('student_login_events')
    .select('user_id,logged_in_at,device_type')
    .in('user_id', userIds)
  if (start) loginQuery = loginQuery.gte('logged_in_at', start.toISOString())
  if (end) loginQuery = loginQuery.lt('logged_in_at', end.toISOString())
  const loginResult = await loginQuery.order('logged_in_at', { ascending: false })
  if (!loginResult.error) {
    events.push(...(loginResult.data || []).map(item => ({
      user_id: item.user_id,
      event_at: item.logged_in_at,
      activity_type: 'login',
      device_type: item.device_type || 'bilgisayar'
    })))
  } else if (!String(loginResult.error.message || '').toLowerCase().includes('does not exist')) {
    console.warn('Eski giriş kayıtları alınamadı', loginResult.error)
  }

  return events.sort((a, b) => new Date(b.event_at) - new Date(a.event_at))
}
...
const STUDENT_LIST_PDF_FIELDS = [
  ...
  ['blank1', 'Boş 1'],
  ['blank2', 'Boş 2'],
  ['lastLogin', 'En Son Aktiflik Tarihi']
]
...
export default function StudentsPage() {
  ...
  async function loadTodayLogins(classId = selectedClass) {
    ...
    try {
      if (!classId) { setTodayLogins([]); return }
      ...
      const [storedEvents, classResult, scienceState, lgsState] = await Promise.all([
        fetchStudentActivityEvents(userIds, { start, end }),
        supabase.from('classes').select('id,name').eq('id', classId).maybeSingle(),
        readSharedState('exams-v1', []).catch(() => ({ payload: [] })),
        readSharedState('lgs-online-exams-v1', []).catch(() => ({ payload: [] }))
      ])

      const studentByUser = new Map((studentRows || []).map(item => [item.auth_user_id, item]))
      const studentById = new Map((studentRows || []).map(item => [String(item.id), item]))
      const studentByNumber = new Map((studentRows || []).map(item => [String(item.student_number), item]))
      ...
      // Bugün bu sürüm kurulmadan önce online deneme çözenleri de geriye dönük aktif say.
      for (const exam of Array.isArray(scienceState?.payload) ? scienceState.payload : []) {
        if (exam?.kind !== 'online') continue
        for (const [studentId, attempt] of Object.entries(exam?.attempts || {})) {
          const student = studentById.get(String(studentId))
          const eventAt = attempt?.finishedAt || attempt?.startedAt
          if (!student || !inToday(eventAt)) continue
          events.push({ user_id: student.auth_user_id, event_at: eventAt, activity_type: 'science_exam', device_type: 'bilgisayar' })
        }
      }
      for (const exam of Array.isArray(lgsState?.payload) ? lgsState.payload : []) {
        for (const participant of exam?.participants || []) {
          const student = studentById.get(String(participant?.studentId)) || studentByNumber.get(String(participant?.studentNumber))
          const eventAt = participant?.finishedAt || participant?.startedAt
          if (!student || !inToday(eventAt)) continue
          events.push({ user_id: student.auth_user_id, event_at: eventAt, activity_type: 'lgs_exam', device_type: 'bilgisayar' })
        }
      }
      ...
      for (const event of events) {
        ...
        if (!current) {
          grouped.set(event.user_id, {
            ...
            last_login_at: event.event_at,
            device_type: event.device_type || 'bilgisayar',
            login_count: 1,
            last_activity_type: event.activity_type || 'app_active'
          })
        } else {
      ...
      console.warn('Bugünkü aktif öğrenciler alınamadı', err)
      setTodayLogins([])
    } finally {
  ...
  async function loadInactiveStudents(classIds) {
    ...
    try {
      ...
      const userIds = [...new Set((studentRows || []).map(item => item.auth_user_id).filter(Boolean))]
      const events = userIds.length ? await fetchStudentActivityEvents(userIds) : []

      const latestByUser = new Map()
      const activityCountByUser = new Map()
      for (const event of events) {
        if (!latestByUser.has(event.user_id)) latestByUser.set(event.user_id, event.event_at)
        activityCountByUser.set(event.user_id, (activityCountByUser.get(event.user_id) || 0) + 1)
      }
      ...
      const rows = (studentRows || []).map(student => {
        ...
        return {
          ...
          login_count: activityCountByUser.get(student.auth_user_id) || 0,
          elapsed
        }
      ...
      console.warn('Aktif olmayan öğrenciler alınamadı', err)
      setInactiveStudents([])
    } finally {
  ...
  function formatPdfLoginDate(value) {
    if (!value) return 'Aktiflik kaydı yok'
  ...
  async function downloadStudentListPdf() {
    ...
    try {
      ...
      // Sınıf listesi her zaman TEK A4 sayfaya sığdırılır.
      // Kalabalık sınıflarda sayfa otomatik yataya döner; satır/font boyutu öğrenci sayısına göre küçülür.
      const useLandscape = rows.length > 55
      const orientation = useLandscape ? 'landscape' : 'portrait'
      const doc = new jsPDF({ unit: 'mm', format: 'a4', orientation, compress: true })
      const pageWidthPx = useLandscape ? 1123 : 794
      const pageHeightPx = useLandscape ? 794 : 1123
      const renderScale = 2
      const marginX = useLandscape ? 24 : 30
      const tableTop = useLandscape ? 86 : 106
      const tableBottom = pageHeightPx - 28
      const headerHeight = useLandscape ? 20 : 24
      const rowsPerPage = Math.max(1, rows.length)
      const rowHeight = Math.max(4.7, (tableBottom - tableTop - headerHeight) / rowsPerPage)
      const pages = [rows]
      ...
      pages.forEach((pageRows, pageIndex) => {
        ...
        const logoSize = useLandscape ? 38 : 44
        if (logoImage) ctx.drawImage(logoImage, marginX, useLandscape ? 10 : 14, logoSize, logoSize)

        ctx.fillStyle = '#142621'
        ctx.font = `700 ${useLandscape ? 18 : 20}px Arial, sans-serif`
        ctx.textBaseline = 'alphabetic'
        ctx.fillText(`${selectedClassName} Sınıf Listesi`, logoImage ? marginX + logoSize + 10 : marginX, useLandscape ? 31 : 40)

        ctx.fillStyle = '#5a5a5a'
        ctx.font = `400 ${useLandscape ? 8.5 : 10}px Arial, sans-serif`
        ctx.fillText(`TAŞKIN • ${new Date().toLocaleDateString('tr-TR')} • ${rows.length} öğrenci`, logoImage ? marginX + logoSize + 10 : marginX, useLandscape ? 47 : 57)

        ctx.strokeStyle = '#178b58'
        ctx.lineWidth = 3
        ctx.beginPath()
        const dividerY = useLandscape ? 60 : 74
        ctx.moveTo(marginX, dividerY)
        ctx.lineTo(pageWidthPx - marginX, dividerY)
        ctx.stroke()

        let x = marginX
        ctx.font = `700 ${useLandscape ? 7.2 : 9.5}px Arial, sans-serif`
        ...
        const bodyFontSize = Math.max(4.6, Math.min(useLandscape ? 7.2 : 9.2, rowHeight * 0.52))
        ...
        if (pageIndex > 0) doc.addPage('a4', orientation)
        doc.addImage(imageData, 'JPEG', 0, 0, useLandscape ? 297 : 210, useLandscape ? 210 : 297, undefined, 'FAST')
      })
  ...
  return (
    <Box>
      ...
      <Paper variant="outlined" sx={{ p: 1.5, mb: 2, borderRadius: 3, background: '#f8fbfa' }}>
        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5} alignItems={{ xs: 'stretch', sm: 'center' }}>
          ...
          <Typography variant="body2" color="text.secondary">
            Aktiflik kartları ve öğrenci listesi seçtiğin aktif sınıfa göre güncellenir.
          </Typography>
        </Stack>
      </Paper>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: '1fr 1fr' }, gap: 2, mb: 2 }}>
        <Paper variant="outlined" sx={{ p: 2, borderRadius: 4, borderColor: '#c9d8e8', background: '#fff' }}>
          <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} alignItems={{ xs: 'stretch', sm: 'center' }}>
            ...
            <Box sx={{ flex: 1 }}>
              <Typography fontWeight={950}>Bugün Aktif Olan Öğrenciler</Typography>
              <Typography variant="h4" fontWeight={950}>{todayLoginsLoading ? '…' : todayLogins.length}</Typography>
              <Typography variant="body2" color="text.secondary">
                {todayLogins.length
                  ? `Son aktif: ${todayLogins[0].first_name} ${todayLogins[0].last_name} • ${loginTime(todayLogins[0].last_login_at)}`
                  : 'Bugün henüz aktif öğrenci yok.'}
              </Typography>
            </Box>
            <Stack direction="row" spacing={1}>
              <IconButton aria-label="Aktiflikleri yenile" onClick={() => { loadTodayLogins(selectedClass); loadInactiveStudents([selectedClass]) }} disabled={todayLoginsLoading}><Refresh /></IconButton>
        ...
        <Paper variant="outlined" sx={{ p: 2, borderRadius: 4, borderColor: '#fecaca', background: '#fffafb' }}>
          <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} alignItems={{ xs: 'stretch', sm: 'center' }}>
            ...
            <Box sx={{ flex: 1 }}>
              <Typography fontWeight={950}>1 Haftadır Aktif Olmayanlar</Typography>
      ...
      <Dialog open={todayLoginsOpen} onClose={() => setTodayLoginsOpen(false)} fullWidth maxWidth="md">
        <DialogTitle fontWeight={950}>{selectedClassName} — Bugün Aktif Olan Öğrenciler ({todayLogins.length})
        ...
        <DialogContent>
          <TableContainer component={Paper} variant="outlined" sx={{ maxHeight: '62vh' }}>
            <Table stickyHeader size="small">
              <TableHead><TableRow><TableCell>Saat</TableCell><TableCell>Öğrenci</TableCell><TableCell>Sınıf</TableCell><TableCell>Cihaz</TableCell><TableCell align="center">Aktiflik</TableCell></TableRow></TableHead>
              <TableBody>
      ...
      >
        <DialogTitle fontWeight={950} sx={{ flex: '0 0 auto', pr: 7 }}>{selectedClassName} — 1 Haftadır Aktif Olmayan Öğrenciler ({inactiveStudents.length})
        ...
        <DialogContent sx={{ display: 'flex', flexDirection: 'column', minHeight: 0, overflow: 'hidden', pt: 1 }}>
          <Alert severity="warning" sx={{ mb: 1.5, flex: '0 0 auto' }}>Henüz aktiflik kaydı olmayan öğrenciler de bu listede gösterilir.</Alert>
          <TextField
            select
            size="small"
            label="Sırala"
          ...
          >
            <MenuItem value="least-logins">En az aktif olanlar</MenuItem>
            <MenuItem value="never-first">Hiç aktif olmayanlar önce</MenuItem>
          ...
          <TableContainer component={Paper} variant="outlined" sx={{ flex: '1 1 auto', minHeight: 0, overflowY: 'auto', WebkitOverflowScrolling: 'touch' }}>
            <Table stickyHeader size="small">
              <TableHead><TableRow><TableCell>Öğrenci</TableCell><TableCell>Sınıf</TableCell><TableCell align="center">Aktiflik</TableCell><TableCell>Son Aktiflik</TableCell><TableCell align="center">Durum</TableCell></TableRow></TableHead>
              <TableBody>
                {sortedInactiveStudents.map(item => <TableRow key={item.id}>
                  <TableCell>{item.student_number} — {item.first_name} {item.last_name}</TableCell>
                  <TableCell>{item.class_name}</TableCell>
                  <TableCell align="center"><b>{item.login_count || 0}</b></TableCell>
                  <TableCell>{item.last_login_at ? new Intl.DateTimeFormat('tr-TR', { dateStyle: 'short', timeStyle: 'short' }).format(new Date(item.last_login_at)) : 'Aktiflik kaydı yok'}</TableCell>
                  <TableCell align="center"><Chip color="error" size="small" label={item.days_inactive == null ? 'Aktiflik kaydı yok' : `${item.days_inactive} gündür`} /></TableCell>
                </TableRow>)}
              </TableBody>
            </Table>
          </TableContainer>
