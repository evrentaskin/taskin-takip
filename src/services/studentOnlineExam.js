...
import { recordStudentActivity } from './studentActivity'
...
export async function saveMyScienceOnlineAttempt(examId, attempt) {
  await recordStudentActivity(attempt?.finishedAt ? 'science_exam_finished' : 'science_exam_active')
...
export async function saveMyLgsOnlineAttempt(examId, participant) {
  await recordStudentActivity(participant?.finishedAt ? 'lgs_exam_finished' : 'lgs_exam_active')
